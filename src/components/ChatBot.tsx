    ...
    <>
      ...
      <div className="fixed bottom-6 right-6 z-50 flex flex-col items-end gap-3">
        ...
        <button
          ...
          className={`relative w-14 h-14 rounded-full flex items-center justify-center shadow-2xl transition-all duration-300 ${isOpen
      ...
      {/* === CHAT WINDOW === */}
      <div
        className={`fixed inset-0 sm:inset-auto sm:bottom-24 sm:right-6 z-[60] w-full sm:w-[380px] transition-all duration-300 origin-bottom-right flex flex-col sm:block ${isOpen
          ? "opacity-100 scale-100 pointer-events-auto"
          : "opacity-0 scale-90 pointer-events-none"
          }`}
      >
        <div className="bg-white dark:bg-[#0d1117] sm:rounded-2xl shadow-2xl border-t sm:border border-slate-200 dark:border-slate-800 overflow-hidden flex flex-col flex-1 sm:h-[520px] sm:max-h-[80vh]"
        >
          {/* Header */}
          ...
          <div className="p-3 border-t border-slate-100 dark:border-slate-800">
            <div className="flex items-center gap-2 bg-slate-50 dark:bg-slate-900 rounded-xl px-3 py-2 border border-slate-200 dark:border-slate-700 focus-within:border-blue-500 focus-within:ring-2 focus-within:ring-blue-500/20 transition-all">
              <input
                ...
                className="flex-1 bg-transparent text-base sm:text-sm text-slate-900 dark:text-white placeholder-slate-400 focus:outline-none disabled:opacity-50"
                maxLength={500}
              />
              <button
                onClick={sendMessage}
